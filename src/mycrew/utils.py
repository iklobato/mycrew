...
class RepoContextBuilder:
    """Builds shared context string for agents."""
...
class RepoDetector:
    """Detects repository information from filesystem."""

    @staticmethod
    def detect_path(cwd: str | None = None) -> str:
...
class RepoCloner:
    ...
    @staticmethod
    def clone(github_repo: str, parent_dir: str, branch: str, token: str) -> str:
        ...
        if not owner or not repo_name:
            raise ValueError("github_repo must be owner/repo format")
        url = (
            f"https://x-access-token:{token.strip()}@github.com/{owner}/{repo_name}.git"
        )
...
class GitHubDetector:
    ...
    @staticmethod
    def detect_repo(repo_path: str) -> str:
    ...
    def derive_issue_url(github_repo: str, issue_id: str) -> str:
        """Build GitHub issue URL from repo and issue_id."""
...
def build_repo_context(
    ...
    """Build a shared context string for agents."""
    return RepoContextBuilder.build(repo_path, github_repo, issue_url)


def detect_repo_path(cwd: str | None = None) -> str:
    """Resolve repo path."""
    return RepoDetector.detect_path(cwd)


def is_git_repo(path: str) -> bool:
    """Return True if path is a valid git repository root."""
    return RepoDetector.is_git_repo(path)
...
def clone_repo_for_issue(
    ...
    """Clone the repo into parent_dir."""
    return RepoCloner.clone(github_repo, parent_dir, branch, token)


def delete_cloned_repo(path: str) -> None:
    """Remove the cloned repo directory."""
    RepoCloner.delete(path)


def detect_github_repo(repo_path: str) -> str:
    """Parse owner/repo from git remote origin."""
    return GitHubDetector.detect_repo(repo_path)


def derive_issue_url(github_repo: str, issue_id: str) -> str:
    """Build GitHub issue URL from repo and issue_id."""
    return GitHubDetector.derive_issue_url(github_repo, issue_id)
